...
# normalize_ocr_text inlined below (from ocr_text.py)
...
def ocr_metrics(
    ...
    for pred, gt in zip(predictions, ground_truths):
        ...
        # BOTH sides (defensive: callers that skip the eval-side GT view still get
...
# =========================================================================
# Multi-OCR set F1 (rich per-player ``ocr_items`` lists)
# =========================================================================
...
def multi_ocr_metrics(
    ...
    ``>= fuzzy_threshold``. Captures *dense* OCR coverage in the rich-OCR schema
...
def sport_type_accuracy(
    ...
    for pred, gt in zip(predictions, ground_truths):
        ...
        if not gt_label:
            # No GT sport label on this frame. Skip from accuracy denominator.
            continue
        total += 1
